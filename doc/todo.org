...
** CODE tick
*** CODE master-tick thread function
    Needs to:
    - [ ] Take threads from a global thread hash ref
    - [ ] catch and log thread exceptions
    - [ ] log if tick fn execution takes longer then the tick sleep time.
