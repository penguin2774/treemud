#+TODO: DESIGN CODE TEST BUG | DONE 
...
***** DONE Putting objects in container in player's inventory
**** DONE give
*** DONE empty
...
**** DONE eat <object>
     Items can have a key :food
     :food {:calories 500 :effect namespace/effect-fn...}
     eat destroys the object and increments calories by the amount
     given if the object has :food data.

**** DONE basic liquids system
     (:liquid obj) presence means obj is a liquid containing object
     Structure:
     {:volume ml-volume in the container
      :hydration amount on drink hydrates you
      :effect affect drinking has on you. (need an effect system...)
**** TEST drink <object>

**** CODE tick fn that causes hunger and thirst
...
** hunger
   over time the players :feed rating should reduce.
   If it reaches 0, the character should receive a penalty or lose health
** damage
